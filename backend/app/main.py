...
def create_product(payload: ProductCreate, session: Session = Depends(db)):
    if payload.stock_quantity < 0: raise HTTPException(400, "Stock cannot be negative")
...
@app.patch("/api/inventory/{product_id}", response_model=Product)
def update_inventory(product_id: str, payload: StockUpdate, session: Session = Depends(db)):
    if payload.stock_quantity < 0: raise HTTPException(400, "Stock cannot be negative")
    session.execute(text("update inventory set stock_quantity=:stock, reorder_level=coalesce(:level, reorder_level), updated_at=now() where product_id=:id"), {"stock": payload.stock_quantity, "level": payload.reorder_level, "id": product_id})
    session.commit()
...
def add_cart_item(user_id: str, payload: CartItemCreate, session: Session = Depends(db)):
    ...
    if stock is None or stock < payload.quantity: raise HTTPException(400, "Insufficient stock")
...
def create_order(payload: OrderCreate, session: Session = Depends(db)):
    ...
    for i in items:
        stock = session.execute(text("select stock_quantity from inventory where product_id=:id"), {"id": i['product_id']}).scalar_one()
        if stock < i['quantity']: raise HTTPException(400, "One or more items are out of stock")
    ...
    for i in items:
        ...
        session.execute(text("update inventory set stock_quantity=stock_quantity-:qty,updated_at=now() where product_id=:product"), {"qty":i['quantity'],"product":i['product_id']})
